...
namespace PizzaOut
{
    public partial class MenuCatItemsViewController_old : UITableViewController
    {
        ...
        public MenuCatItemsViewController_old(MenuCategory menuCat)
        {
        ...
        public MenuCatItemsViewController_old (IntPtr handle) : base (handle)
        {
        }
        ...
        {
            base.ViewDidLoad();
            string menuTitle = "Test";///_selectedCategory.MENU_CAT_NAME+" Menu";

            Title = textInfo.ToTitleCase(textInfo.ToLower(menuTitle));

            _menuCategoriesItems = await LoadMenuCatItems(2);///(_selectedCategory.MENU_CAT_ID);
